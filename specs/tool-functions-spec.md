...
def lookup_plant(plant_name):
    ...
    # Alias match
    for plant in _plant_db.values():
        aliases = [alias.lower() for alias in plant["aliases"]]
    ...
    return {
        "found": False,
        "name": normalized,
        "message": f"No plant matching '{normalized}' found in the database. Ask the user for a more specific common name, scientific name, or alternate spelling and try again."
    }
### Design Decisions
...
```
Loop through each plant in _plant_db. Convert each alias in plant["aliases"] to lowercase and compare it to the normalized input. If any alias matches, return that plant.
```
...
```
No plant matching '{normalized}' found in the database. Ask the user for a more specific common name, scientific name, or alternate spelling and try again.
```
...
```
Users may enter plant names with extra spaces or different capitalization. Using strip() and lower() ensures these inputs still match correctly.
```
